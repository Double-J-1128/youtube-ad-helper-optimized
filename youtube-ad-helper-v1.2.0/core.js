...
((root, factory) => {
  ...
  // 保留该常量供旧代码兼容；Infinity 表示不再设置人为广告位数量上限。
  ...
  function buildSchedule({
    ...
    startTime,
    interval,
    endBuffer = DEFAULT_END_BUFFER_SECONDS,
    maxSlots = MAX_SLOTS
  }) {
    if (![duration, startTime, interval, endBuffer].every(Number.isFinite)) {
      return { ok: false, error: '生成计划所需的参数无效', times: [] };
    }
    if (maxSlots !== Number.POSITIVE_INFINITY && (!Number.isFinite(maxSlots) || maxSlots <= 0)) {
      return { ok: false, error: '广告位数量参数无效', times: [] };
    }
    if (duration < MIN_VIDEO_DURATION_SECONDS) {
      return { ok: false, error: '视频不足 8 分钟', times: [] };
    }
    if (startTime < 0 || interval <= 0 || endBuffer < 0) {
    ...
    const totalCount = Math.floor((lastAllowed - startTime) / interval) + 1;
    const outputCount = Math.min(totalCount, maxSlots);
    const times = new Array(outputCount);
    for (let index = 0; index < outputCount; index += 1) {
      times[index] = startTime + index * interval;
    }
  ...
  function parseAnyTime(value) {
    if (typeof value !== 'string') return null;
    const clock = value.match(/\d+:\d{2}:\d{2}(?::\d{2})?/);
    if (clock) {
