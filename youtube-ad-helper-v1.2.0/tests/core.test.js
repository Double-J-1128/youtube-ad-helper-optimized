...
test('默认计划不再受 100 个广告位限制', () => {
  const plan = Core.buildSchedule({ duration: 108000, startTime: 0, interval: 300, endBuffer: 0 });
  assert.equal(plan.ok, true);
  assert.equal(plan.times.length, 361);
  assert.equal(Core.MAX_SLOTS, Number.POSITIVE_INFINITY);
});

test('仍可由调用方显式设置临时数量限制', () => {
...
test('解析页面时间并拒绝无效秒数', () => {
  ...
  assert.equal(Core.parseDisplayTime('10000:03:00'), 600003);
